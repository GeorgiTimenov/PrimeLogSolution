...
if (!class_exists('Contact')) {
    ...
    class Contact
    {
        ...
        private function __construct()
        {
             ...
             */
            $this->_autoResponder   =   'info@primelogsolutions.com';
             ...
             */
            $this->_htmlMailTemplate = '<html ><body style="margin:10px 0 0 0">';
            $this->_htmlMailTemplate .= '<table style="margin:auto;padding:20px;width:90%;display:block;color:#000;background-color:#dddfea;font-family:Tahoma;border-radius:10px"><tr>';
            $this->_htmlMailTemplate .= '<td style="padding:5px;width:100%;display:block">';
             $this->_htmlMailTemplate .= '<tr><td>Name : {userFullName}</td></tr>';
            $this->_htmlMailTemplate .= '<tr><td>E-Mail : {userEmail}</td></tr>';
            // check for optional fields and add them if they are set
            if (isset($_POST['phone']) and (trim($_POST['phone']) !== '')) {
                $this->_htmlMailTemplate .= '<tr><td>Phone : {userPhone}</td></tr>';
				$this->_htmlMailTemplate .= '<hr /><p>{userMessage}</p>';
            ...
            $this->_replyHtmlMailTemplate .= '<tr><td style="width:90%;display:block">';
            $this->_replyHtmlMailTemplate .= 'Dear&nbsp;{userName}</td></tr>';
            $this->_replyHtmlMailTemplate .= '<tr><td><p>We just received your contact information.<br />We\'ll reach you as soon as possible.</p><br /></td></tr>'; 
        ...
        public function sendMail()
        {
           ...
           if (isset($submittedData['phone']) and !empty($submittedData['phone']) and (!preg_match('/^(\+[1-9][0-9]*(\([0-9]*\)|-[0-9]*-))?[0]?[1-9][0-9\- ]*$/', $submittedData['phone']))) {
            ...
            $headers[]			=	"{$submittedData['contact_subject']}";
            /*
            ...
            if ($mailed) {
                ...
                //if (isset($submittedData['acknowledge']) and ($submittedData['acknowledge']==='1')) {
                     ...
                     * Mail it and catch the result to further check if mail has sent or not
                     */
                    $mail			=	mail($submittedData['contact_email'], ' ' . $submittedData['contact_subject'], $message, $finalHeaderString);
                //}
 ...
 */
$emails                             = 'info@primelogsolutions.com, clavelromano@gmail.com';
/**
 ...
 */
$autoResponder                      =   'Info@primelogsolutions.com';
